...
"""
This script contains the train loop for training the UNet
"""
...
def train_one_epoch(epoch, dataloader, device, model, criterion, optimizer):
    ...
    for image, target in tqdm(dataloader, "iterating over train batches..."): 
        ...
        optimizer.step()
    visualize_sample(image, target, pred, epoch, save=True)
    avg_loss = train_loss / num_train_batches
    return avg_loss


def eval_one_epoch(dataloader, device, model, criterion):
    """
...
def train(num_epochs, train_loader, val_loader, device, model, criterion, optimizer):
    ...
    for epoch in range(num_epochs):
        ...
        val_loss = eval_one_epoch(val_loader, device, model, criterion)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--lr", default=0.0001, help='specify the learning rate')
